//C:\Users\user\RideSharing\src\components\Layout\TaiwanMap.vue
<template>
  <div class="taiwna-map">
    <svg 
      ref="mapContainer"
      class="map-svg"
...
// 定義 props（接收父元件傳來的 scale 和 translate）
...
const translate = computed(() => {
  return Array.isArray(props.translate) && props.translate.length === 2
    ? props.translate
    : [-12, -12] // 如果傳進來的有問題，就用預設值
})

// 組出 SVG 的 transform 樣式
const svgTransformStyle = computed(() => {
  return `
    transform: scale(${props.scale}) translate(${translate.value[0]}%, ${translate.value[1]}%);
